...
- CI/CD pipelines""",
                        ...
                        with gr.Tabs():
                            with gr.Tab("Grid View"):
                                candidates_grid = gr.Dataframe(
                                    headers=["Name", "Email", "Match Score", "Skills", "Experience"],
                ...
                def format_candidate_grid(search_results):
                    ...
                    for idx, match in enumerate(search_results["matching_candidates"]):
                        ...
                        # Get match score and ensure it's between 50% and 100%
                        ...
                        if isinstance(match_score, (int, float)):
                            ...
                            if score > 1:
                                score = score / 100
                            # Ensure minimum score is 50%
                            score = max(0.5, min(1.0, score))
                            score_display = f"{score*100:.1f}%"
                        else:
                            score_display = "N/A"
                        
                        print(f"Formatted row: [{name}, {email}, {score_display}, {skills_display}, {experience}]")
                        grid_data.append([
                            name,
                            email,
                            score_display,
...
- CI/CD pipelines""",
                ...
                def format_candidate_grid(search_results):
                    ...
                    for idx, match in enumerate(search_results["matching_candidates"]):
                        ...
                        match_score = analysis.get("overall_score", 0)
                        if isinstance(match_score, (int, float)):
                            score_display = f"{match_score*100:.1f}%"
                        else:
                            score_display = "N/A"
